...
function DrawTextBox(textDataTable, maxSizeBoxOrCenterPoint, config)
	...
	FrameMan:DrawBoxFillPrimitive(Vector(topLeft.X - config.boxPadding, topLeft.Y - config.boxPadding), Vector(topLeft.X + maxSize.X + config.boxPadding, topLeft.Y + maxSize.Y + config.boxPadding), config.boxOutlineColour);
	FrameMan:DrawBoxFillPrimitive(Vector(topLeft.X - config.boxPadding + config.boxOutlineWidth, topLeft.Y - config.boxPadding + config.boxOutlineWidth), Vector(topLeft.X + maxSize.X + config.boxPadding - config.boxOutlineWidth, topLeft.Y + maxSize.Y + config.boxPadding - config.boxOutlineWidth), config.boxBGColour);
	
	for i, textDataEntry in ipairs(textDataTable) do
		local linePos = Vector(topLeft.X, topLeft.Y + (i - 1) * LINE_HEIGHT[config.useSmallText]);
		...
		FrameMan:DrawTextPrimitive(linePos, textDataEntry.text, config.useSmallText, alignment);
	end
end
...
function _localUIFunctions:getScaledMaxSizeAndFixTextDataTableIfNecessary(textDataTable, maxSize, config)
	...
	local totalHeight = numberOfLines * LINE_HEIGHT[config.useSmallText];
	for i = #textDataTable, 1, -1 do
		if (totalHeight > maxSize.Y) then
			totalHeight = totalHeight - LINE_HEIGHT[config.useSmallText] * _localUIFunctions:getNumberOfLinesForText(textDataTable[i].text, maxSize.X, config.useSmallText);
